...
def pool_2d_nxn_regions(inputs, output_size:int, mode:str):
    inputs_shape = tf.shape(inputs)
    b = tf.cast(inputs_shape[0], tf.int32)
    h = tf.cast(inputs_shape[1], tf.int32)
    w = tf.cast(inputs_shape[2], tf.int32)
    c = tf.cast(inputs_shape[3], tf.int32)
    ...
    if mode == 'max':
        ...
        msg = "Mode must be either 'max' or 'ave'"
    ...
    result = tf.reshape(result, [b, output_size, output_size, 512])
    return result
...
def vgg_16(inputs,
  ...
  with tf.variable_scope(scope, 'vgg_16', [inputs]) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
        ...
        #net = slim.conv2d(net, 4096, [3, 3], scope='fc6')
        #net = slim.conv2d(net, 4096, [3, 3], scope='fc7')
        #end_points['fc7'] = net

        # fixed size pooling
        net = pool_2d_nxn_regions(net, 6, 'max')
        net = slim.conv2d(net, 4096, [1, 1], scope='fc6')
        net = slim.conv2d(net, 4096, [1, 1], scope='fc7')

        net = tf.reshape(net, [-1, 36, 1, 4096])
